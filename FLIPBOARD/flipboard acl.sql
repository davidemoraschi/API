...
BEGIN
	DBMS_NETWORK_ACL_ADMIN.drop_acl (acl => 'flipboard.xml');
END;
/

BEGIN
	--DBMS_NETWORK_ACL_ADMIN.drop_acl (acl => 'harvest_harvest.xml');
	DBMS_NETWORK_ACL_ADMIN.create_acl (acl => 'flipboard.xml'
																		,description => 'Flipboard HTTP Access'
																		,principal => 'FLIPBOARD'
																		,is_grant => TRUE
																		,PRIVILEGE => 'connect'
																		,start_date => NULL
																		,end_date => NULL);
	DBMS_NETWORK_ACL_ADMIN.add_privilege (acl => 'flipboard.xml'
																			 ,principal => 'FLIPBOARD'
																			 ,is_grant => TRUE
																			 ,PRIVILEGE => 'resolve'
																			 ,start_date => NULL
																			 ,end_date => NULL);
	DBMS_NETWORK_ACL_ADMIN.assign_acl (acl => 'flipboard.xml', HOST => '*.flipboard.com');
