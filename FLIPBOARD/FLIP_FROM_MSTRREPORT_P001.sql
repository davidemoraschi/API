...
BEGIN
	DBMS_SCHEDULER.DROP_JOB (job_name => 'FLIP_FROM_MSTRREPORT_P001');
END;

ALTER SESSION SET TIME_ZONE = 'Europe/Madrid';

BEGIN
	DBMS_SCHEDULER.create_job (job_name => 'FLIP_FROM_MSTRREPORT_P001'
														...
														,job_action => 'BEGIN sys.p_ftp_put_binary(to_char(sysdate,''YYYY-MM-DD'')||'' HelloWorld.html''); p_flip_from_url (p_url => ''http://pandas10.azurewebsites.net/'||to_char(sysdate,'YYYY-MM-DD')||'%20HelloWorld.html'', p_user  => ''EuroStrategy'', p_pass => ''3SamsungGalaxySII'', p_target_magazine =>''flipboard/mag-M4sfaY1mTfWWyp4EpnhHNQ%3Am%3A120774391-123011160''); END;'
