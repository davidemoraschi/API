...
CREATE OR REPLACE TYPE TWITTER.TWITTER
   UNDER OAUTH.OAUTH
...
CREATE TABLE TWITTER.OBJS_TWITTER (ACCOUNT VARCHAR2 (50 BYTE), CREATION_DATE TIMESTAMP (6) WITH TIME ZONE, OBJ_TWITTER TWITTER.TWITTER)
COLUMN OBJ_TWITTER NOT SUBSTITUTABLE AT ALL LEVELS;

ALTER TABLE TWITTER.OBJS_TWITTER ADD (
  CONSTRAINT OBJECTS_TWITTER_PK
 PRIMARY KEY
 (ACCOUNT));

CREATE OR REPLACE TYPE BODY TWITTER.TWITTER
AS
   CONSTRUCTOR FUNCTION TWITTER (id                      IN VARCHAR2 DEFAULT 'test',
                                 oauth_consumer_key      IN VARCHAR2 DEFAULT NULL,
                                 oauth_consumer_secret   IN VARCHAR2 DEFAULT NULL,
