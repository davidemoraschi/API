/* Formatted on 13/03/2014 11:30:49 (QP5 v5.163.1008.3004) PANDAS Copyright (c) 2013 -Davide Moraschi (davidem@eurostrategy.net)
Todos los derechos reservados. Prohibida su reproducción Total o Parcial. */
BEGIN
	DBMS_EPG.drop_dad (dad_name => 'odata');
END;
/

BEGIN
	DBMS_EPG.create_dad (dad_name => 'odata', PATH => '/odata/*');
	DBMS_EPG.authorize_dad (dad_name => 'odata', USER => 'ODATA');
	DBMS_EPG.set_dad_attribute (dad_name => 'odata', attr_name => 'default-page', attr_value => 'home.htm');
	DBMS_EPG.set_dad_attribute (dad_name => 'odata', attr_name => 'database-username', attr_value => 'ODATA');
	DBMS_EPG.set_dad_attribute (dad_name => 'odata', attr_name => 'nls-language', attr_value => 'american_america.al32utf8');
	DBMS_EPG.authorize_dad (dad_name => 'odata', USER => 'ODATA');
END;
/
...
CREATE OR REPLACE PACKAGE ODATA.home IS
	PROCEDURE htm;
END home;
/

CREATE OR REPLACE PACKAGE BODY ODATA.home
IS
   PROCEDURE htm
   IS
   BEGIN
