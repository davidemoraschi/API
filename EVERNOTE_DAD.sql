...
BEGIN
   DBMS_EPG.drop_dad (dad_name => 'evernote');
END;
/

BEGIN
   DBMS_EPG.create_dad (dad_name => 'evernote', PATH => '/evernote/*');
   DBMS_EPG.authorize_dad (dad_name => 'evernote', USER => 'EVERNOTE');
   DBMS_EPG.set_dad_attribute (dad_name => 'evernote', attr_name => 'default-page', attr_value => 'home.htm');
   DBMS_EPG.set_dad_attribute (dad_name => 'evernote', attr_name => 'database-username', attr_value => 'EVERNOTE');
   DBMS_EPG.set_dad_attribute (dad_name => 'evernote', attr_name => 'nls-language', attr_value => 'american_america.al32utf8');
   DBMS_EPG.authorize_dad (dad_name => 'evernote', USER => 'EVERNOTE');
END;
/
...
CREATE OR REPLACE PACKAGE EVERNOTE.home
IS
   PROCEDURE htm;
END home;
/

CREATE OR REPLACE PACKAGE BODY EVERNOTE.home
IS
   PROCEDURE htm
   IS
   BEGIN
