...
BEGIN
   DBMS_EPG.drop_dad (dad_name => 'linkedin');
END;
/

BEGIN
   DBMS_EPG.create_dad (dad_name => 'linkedin', PATH => '/linkedin/*');
   DBMS_EPG.authorize_dad (dad_name => 'linkedin', USER => 'LINKEDIN');
   DBMS_EPG.set_dad_attribute (dad_name => 'linkedin', attr_name => 'default-page', attr_value => 'home.htm');
   DBMS_EPG.set_dad_attribute (dad_name => 'linkedin', attr_name => 'database-username', attr_value => 'LINKEDIN');
   DBMS_EPG.set_dad_attribute (dad_name => 'linkedin', attr_name => 'nls-language', attr_value => 'american_america.al32utf8');
   DBMS_EPG.authorize_dad (dad_name => 'linkedin', USER => 'LINKEDIN');
END;
/
...
CREATE OR REPLACE PACKAGE LINKEDIN.home
IS
   PROCEDURE htm;
END home;
/

CREATE OR REPLACE PACKAGE BODY LINKEDIN.home
IS
   PROCEDURE htm
   IS
   BEGIN
